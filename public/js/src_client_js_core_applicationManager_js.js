...
/***/ "./src/client/js/visualizer/chunk-loader.js":
/*!**************************************************!*\
  !*** ./src/client/js/visualizer/chunk-loader.js ***!
  \**************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   ChunkLoader: () => (/* binding */ ChunkLoader)
/* harmony export */ });
/* harmony import */ var _parser_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./parser.js */ "./src/client/js/visualizer/parser.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
 ...
 */
var ChunkLoader = /*#__PURE__*/function () {
  function ChunkLoader(visualizer) {
   ...
   */
  return _createClass(ChunkLoader, [{
    key: "loadGcodeProgressive",
    value: (function () {
      var _loadGcodeProgressive = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(gcode) {
        var _this = this;
        var options,
          _options$chunkSize,
          chunkSize,
        ...
        return _regenerator().w(function (_context2) {
          while (1) switch (_context2.p = _context2.n) {
            case 0:
              ...
              _options$chunkSize = options.chunkSize, chunkSize = _options$chunkSize === void 0 ? 1000 : _options$chunkSize, _options$onProgress = options.onProgress, onProgress = _options$onProgress === void 0 ? null : _options$onProgress, _options$onChunkProce = options.onChunkProcessed, onChunkProcessed = _options$onChunkProce === void 0 ? null : _options$onChunkProce, _options$onCancel = options.onCancel, onCancel = _options$onCancel === void 0 ? null : _options$onCancel;
              this.isCancelled = false;
              this.isRunning = true;
              ...
              accumulatedModes = [];
              accumulatedPoints = [];
              accumulatedLineMap = []; // MISSING: Accumulate line maps!
              // console.log(
              ...
              currentState = {
                ...
                // Default to linear interpolation (G-code standard)
                anyDrawn: false
              };
              chunkMetaData = []; // Track metadata for debugging
              _loop = /*#__PURE__*/_regenerator().m(function _loop() {
                var chunkIndex, startLine, endLine, chunkLines, chunkResult, chunkStartLine, offsetLineMap;
                return _regenerator().w(function (_context) {
                  while (1) switch (_context.n) {
                    case 0:
                      chunkIndex = i;
                      startLine = chunkIndex * chunkSize;
                      endLine = Math.min((chunkIndex + 1) * chunkSize, lines.length);
                      chunkLines = lines.slice(startLine, endLine); //console.log(`[CHUNK] Processing chunk ${chunkIndex} with ${chunkLines.length} lines`);
                      // Process chunk sequentially to maintain proper state
                      _context.n = 1;
                      return _this.processChunk(chunkLines, chunkIndex, currentState);
                    case 1:
                      chunkResult = _context.v;
                      // Accumulate result
                      if (chunkResult) {
                        ...
                        if (chunkResult.segments) {
                          accumulatedSegments.push.apply(accumulatedSegments, _toConsumableArray(chunkResult.segments));
                          accumulatedModes.push.apply(accumulatedModes, _toConsumableArray(chunkResult.modes));
                          accumulatedPoints.push.apply(accumulatedPoints, _toConsumableArray(chunkResult.points));
                          ...
                          if (chunkResult.lineMap) {
                            ...
                            chunkStartLine = chunkIndex * chunkSize;
                            offsetLineMap = chunkResult.lineMap.map(function (lineNum) {
                              return lineNum >= 0 ? lineNum + chunkStartLine : lineNum;
                            });
                            accumulatedLineMap.push.apply(accumulatedLineMap, _toConsumableArray(offsetLineMap));
                          }
                        }
                        ...
                        chunkMetaData.push({
                          ...
                          segmentCount: chunkResult.segments ? chunkResult.segments.length : 0,
                          initialState: currentState,
                          finalState: chunkResult.finalState || currentState
                        });
                      }
                      processedChunks++;
                      _this.loadingProgress = processedChunks / totalChunks * 100;
                      if (onProgress) {
                        onProgress({
                          progress: _this.loadingProgress,
                      ...
                      if (chunkResult && chunkResult.finalState) {
                        ...
                        console.warn("[CHUNK] No final state from chunk ".concat((chunkResult === null || chunkResult === void 0 ? void 0 : chunkResult.chunkIndex) || 'unknown'));
                      }

                      // Yield to main thread to keep UI responsive
                      _context.n = 2;
                      return _this.yieldToMainThread();
                    case 2:
                      return _context.a(2);
                  }
                }, _loop);
              ...
              i = 0;
            case 2:
              if (!(i < totalChunks && !this.isCancelled)) {
                _context2.n = 4;
                break;
              }
            ...
            case 5:
              ...
              result = {
                ...
                modes: accumulatedModes,
                points: accumulatedPoints,
                lineMap: accumulatedLineMap,
                // MISSING: Include accumulated line map!
            ...
            case 6:
              _context2.p = 6;
              _t = _context2.v;
              console.error('[CHUNK] Loading failed:', _t.message);
              return _context2.a(2, {
                success: false,
                error: _t.message
              });
      ...
      function loadGcodeProgressive(_x) {
        return _loadGcodeProgressive.apply(this, arguments);
      }
      return loadGcodeProgressive;
    }()
    /**
    ...
    key: "processChunk",
    value: (function () {
      var _processChunk = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2(chunkLines, chunkIndex) {
        var _this2 = this;
        var initialState,
          _args3 = arguments;
        return _regenerator().w(function (_context3) {
          while (1) switch (_context3.n) {
            case 0:
              initialState = _args3.length > 2 && _args3[2] !== undefined ? _args3[2] : {};
              return _context3.a(2, new Promise(function (resolve) {
                try {
                  // Use Web Worker for heavy processing if available
                  if (_this2.visualizer.useWebWorkers && window.Worker) {
                    _this2.processChunkWithWorker(chunkLines, chunkIndex, resolve);
                  } else {
                    // Process in main thread
                    var result = _this2.parseGcodeChunk(chunkLines, chunkIndex, initialState);
                    resolve(result);
                  }
                } catch (error) {
                  resolve({
    ...
    key: "processChunkWithWorker",
    value: function processChunkWithWorker(chunkLines, chunkIndex, initialState, resolve) {
      // This would be implemented when we add Web Worker support
      // For now, fallback to main thread processing
      var result = this.parseGcodeChunk(chunkLines, chunkIndex, initialState);
      resolve(result);
    }
    ...
    value: function parseGcodeChunk(chunkLines, chunkIndex) {
      ...
      return {
        ...
        // MISSING: Include the line map from parser!
        ...
        metaData: {
          startIndex: parseResult.lineMap ? Math.min.apply(Math, _toConsumableArray(parseResult.lineMap.filter(function (idx) {
            return idx >= 0;
          }))) : 0,
          endIndex: parseResult.lineMap ? Math.max.apply(Math, _toConsumableArray(parseResult.lineMap.filter(function (idx) {
            return idx >= 0;
          }))) : 0,
    ...
    key: "yieldToMainThread",
    value: function yieldToMainThread() {
    ...
    key: "validateStateContinuity",
    value: function validateStateContinuity(chunkResults) {
      var report = {
        isValid: true,
        issues: [],
        stateTransitions: []
      ...
      for (var i = 0; i < chunkResults.length - 1; i++) {
        var current = chunkResults[i];
        var next = chunkResults[i + 1];
        if (current.finalState && next.metaData) {
          ...
          var positionMismatch = Math.abs(current.finalState.x - next.metaData.initialState.x) > 0.001 || Math.abs(current.finalState.y - next.metaData.initialState.y) > 0.001 || Math.abs(current.finalState.z - next.metaData.initialState.z) > 0.001;
          var modeMismatch = current.finalState.currentMotionMode !== next.metaData.initialState.currentMotionMode;
          if (positionMismatch) {
...
/***/ "./src/client/js/visualizer/controls.js":
/*!**********************************************!*\
  !*** ./src/client/js/visualizer/controls.js ***!
  \**********************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   ViewerControls: () => (/* binding */ ViewerControls),
/* harmony export */   calculateIncrementalSegmentCounts: () => (/* binding */ calculateIncrementalSegmentCounts),
/* harmony export */   resetSeekDebounceState: () => (/* binding */ resetSeekDebounceState),
/* harmony export */   resetSegmentCountState: () => (/* binding */ resetSegmentCountState),
/* harmony export */   runSimulation: () => (/* binding */ runSimulation),
/* harmony export */   setupSimulationControls: () => (/* binding */ setupSimulationControls),
/* harmony export */   stopSimulation: () => (/* binding */ stopSimulation),
/* harmony export */   throttledRender: () => (/* binding */ throttledRender),
/* harmony export */   updateCurrentLineDisplay: () => (/* binding */ updateCurrentLineDisplay),
/* harmony export */   updateDrawRangesBatch: () => (/* binding */ updateDrawRangesBatch),
/* harmony export */   updatePerformanceMonitoring: () => (/* binding */ updatePerformanceMonitoring),
/* harmony export */   updatePlayPauseIcon: () => (/* binding */ updatePlayPauseIcon),
/* harmony export */   updateVisualizationToIndex: () => (/* binding */ updateVisualizationToIndex),
/* harmony export */   validateSegmentCounts: () => (/* binding */ validateSegmentCounts)
/* harmony export */ });
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _defineProperties(e, r) { for (var t = 0; t < r.length; t++) { var o = r[t]; o.enumerable = o.enumerable || !1, o.configurable = !0, "value" in o && (o.writable = !0), Object.defineProperty(e, _toPropertyKey(o.key), o); } }
function _createClass(e, r, t) { return r && _defineProperties(e.prototype, r), t && _defineProperties(e, t), Object.defineProperty(e, "prototype", { writable: !1 }), e; }
...
var seekDebounceState = {
  seekTimeout: null,
  lastSeekPosition: -1,
  isRapidSeeking: false,
  seekStartTime: 0,
  lastInputTime: 0,
  rapidSeekThreshold: 30,
  // OPTIMIZATION: Reduced from 50ms to 30ms for more aggressive rapid seeking detection
  isDragging: false // Track if user is actively dragging
};

// Render throttling state for seeking optimization
var renderThrottleState = {
  lastRenderTime: 0,
  renderTimeout: null,
  maxFPS: 30,
  // Maximum 30 FPS during seeking
  minFrameTime: 1000 / 30,
  // ~33ms between renders
  pendingRender: false
};
...
function detectRapidSeeking(currentTime) {
  var timeSinceLastInput = currentTime - seekDebounceState.lastInputTime;
  seekDebounceState.lastInputTime = currentTime;

  // OPTIMIZATION: Consider dragging state for better rapid seeking detection
  var isInputFrequent = timeSinceLastInput < seekDebounceState.rapidSeekThreshold;
  var isDragging = seekDebounceState.isDragging;
...
function throttledRender() {
  var isSeekingOperation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  ...
  if (isSeekingOperation && seekDebounceState.isRapidSeeking) {
    ...
    var timeSinceLastRender = currentTime - renderThrottleState.lastRenderTime;
    if (timeSinceLastRender < renderThrottleState.minFrameTime) {
      ...
      if (!renderThrottleState.pendingRender) {
        ...
        var delay = renderThrottleState.minFrameTime - timeSinceLastRender;
        renderThrottleState.renderTimeout = setTimeout(function () {
...
function setupSimulationControls(domElements, parseResult) {
  var simState = globalSimState; // Use global state
  var simGeneration = globalSimGeneration;
  ...
  if (domElements.progressBar && parseResult.toolpathSegments) {
    ...
    var debouncedSeekUpdate = debounce(function (newIdx, maxSegments) {
    ...
    domElements.progressBar.oninput = function () {
      var newIdx = Number(domElements.progressBar.value);
      var maxSegments = parseResult.toolpathSegments.length;
      var currentTime = performance.now();

      // Detect if user is rapidly seeking
      var isRapidSeeking = detectRapidSeeking(currentTime);
  ...
  if (domElements.speedSlider && domElements.speedValue) {
    ...
    var speed = Math.max(0.001, Number(domElements.speedSlider.value || 10));
    var updateSpeedLabel = function updateSpeedLabel() {
      if (domElements.speedValue && domElements.speedUnit) {
        if (speed > 1) {
          ...
          var segsPerTick = Math.max(1, Math.round(1 / speed));
  ...
  if (domElements.playBtn) {
    domElements.playBtn.onclick = function () {
      var maxSegments = parseResult.toolpathSegments ? parseResult.toolpathSegments.length : 0;
      var isAtEnd = simState.idx >= maxSegments;
      var isCompleted = simState.stopped && isAtEnd;
      if (simState.playing) {
        ...
        var startIdx = simState.paused ? simState.idx : 0;
        simState.playing = true;
        simState.paused = false;
        simState.stopped = false;
  ...
  if (domElements.forwardBtn) {
    domElements.forwardBtn.onclick = function () {
      var endIdx = parseResult.toolpathSegments.length;
      simState.idx = endIdx;
      simState.playing = false;
      simState.paused = false;
      simState.stopped = true;
...
function updatePlayPauseIcon(domElements, simState) {
  ...
  var playIcon = '<polygon points="4,2 14,8 4,14" fill="#fff"/>';
  var pauseIcon = '<rect x="3" y="2" width="3" height="12" fill="#fff"/><rect x="10" y="2" width="3" height="12" fill="#fff"/>';
  var restartIcon = '<polygon points="4,2 14,8 4,14" fill="#fff"/><circle cx="12" cy="4" r="1.5" fill="#fff"/>';

  // Determine if simulation is completed
  var maxSegments = window.gcodeToolpathSegments ? window.gcodeToolpathSegments.length : 0;
  var isAtEnd = simState.idx >= maxSegments;
  var isCompleted = simState.stopped && isAtEnd;
...
function updateLineInfoDisplay(commandLineNumber, segmentIndex) {
  var infoContainer = document.getElementById('gcodeLineInfo');
  if (!infoContainer) {
    return;
  }

  // Display the command line number, not the segment index
  var displayLineNumber = commandLineNumber >= 0 ? commandLineNumber : Math.max(0, Math.min(segmentIndex, window.gcodeLines.length - 1));
  ...
  if (displayLineNumber >= 0 && displayLineNumber < window.gcodeLines.length) {
    var line = window.gcodeLines[displayLineNumber];
    infoHtml += "<br>#".concat(displayLineNumber + 1, " ").concat(line);
  }
...
function updateEditorSelection(commandLineNumber) {
  ...
  var actualLineNumber = commandLineNumber + 1; // Convert to 1-based
  ...
  window.outputEditor.setPosition({
...
function calculateIncrementalSegmentCounts(targetIdx, parseResult) {
  ...
  var maxIdx = Math.min(targetIdx, parseResult.toolpathModes.length);
  ...
  if (segmentCountState.needsFullRecalc || targetIdx < segmentCountState.lastIndex) {
    ...
    for (var i = 0; i < maxIdx; i++) {
      var mode = parseResult.toolpathModes[i];
      if (mode === 'G0') segmentCountState.counts.G0++;else if (mode === 'G1') segmentCountState.counts.G1++;else if (mode === 'G2') segmentCountState.counts.G2++;else if (mode === 'G3') segmentCountState.counts.G3++;
    }
    ...
    for (var _i = segmentCountState.lastIndex; _i < maxIdx; _i++) {
      var _mode = parseResult.toolpathModes[_i];
      if (_mode === 'G0') segmentCountState.counts.G0++;else if (_mode === 'G1') segmentCountState.counts.G1++;else if (_mode === 'G2') segmentCountState.counts.G2++;else if (_mode === 'G3') segmentCountState.counts.G3++;
    }
    segmentCountState.lastIndex = maxIdx;
  }
  ...
  return _objectSpread({}, segmentCountState.counts);
}
...
function updateDrawRangesBatch(segmentCounts) {
  ...
  var geometries = window.gcodeBatchedGeometries;
  if (geometries.G0 && segmentCounts.G0 !== undefined) {
    var newRange = segmentCounts.G0 * 2;
  ...
  if (geometries.G1 && segmentCounts.G1 !== undefined) {
    var _newRange = segmentCounts.G1 * 2;
    if (lastDrawRanges.G1 !== _newRange) {
      geometries.G1.setDrawRange(0, _newRange);
      lastDrawRanges.G1 = _newRange;
    }
  }
  if (geometries.G2 && segmentCounts.G2 !== undefined) {
    var _newRange2 = segmentCounts.G2 * 2;
    if (lastDrawRanges.G2 !== _newRange2) {
      geometries.G2.setDrawRange(0, _newRange2);
      lastDrawRanges.G2 = _newRange2;
    }
  }
  if (geometries.G3 && segmentCounts.G3 !== undefined) {
    var _newRange3 = segmentCounts.G3 * 2;
    if (lastDrawRanges.G3 !== _newRange3) {
      geometries.G3.setDrawRange(0, _newRange3);
      lastDrawRanges.G3 = _newRange3;
    }
  }
}
...
function updatePerformanceMonitoring() {
  ...
  if (performanceMonitor.lastFrameTime > 0) {
    var frameTime = currentTime - performanceMonitor.lastFrameTime;
    ...
    performanceMonitor.averageFrameTime = performanceMonitor.frameTimeHistory.reduce(function (a, b) {
      return a + b;
    }, 0) / performanceMonitor.frameTimeHistory.length;
  }
  ...
  if (performanceMonitor.frameCount % 60 === 0) {
    var avgFPS = performanceMonitor.averageFrameTime > 0 ? 1000 / performanceMonitor.averageFrameTime : 0;
...
function validateSegmentCounts(targetIdx, parseResult) {
  ...
  for (var i = 0; i < maxIdx; i++) {
    var mode = parseResult.toolpathModes[i];
    if (mode === 'G0') g0Count++;else if (mode === 'G1') g1Count++;else if (mode === 'G2') g2Count++;else if (mode === 'G3') g3Count++;
  }
  var expected = {
    G0: g0Count,
    G1: g1Count,
  ...
  var actual = segmentCountState.counts;
  var isValid = expected.G0 === actual.G0 && expected.G1 === actual.G1 && expected.G2 === actual.G2 && expected.G3 === actual.G3;
  if (!isValid) {
...
function stopSimulation() {
  if (globalSimState.playing) {
    ...
    var playBtn = document.getElementById('runSimulationBtn');
    var runSimulationIcon = document.getElementById('runSimulationIcon');
    if (playBtn && runSimulationIcon) {
      var domElements = {
        playBtn: playBtn,
        runSimulationIcon: runSimulationIcon
      };
...
function updateVisualizationToIndex(targetIdx, parseResult) {
  ...
  var segmentCounts = calculateIncrementalSegmentCounts(targetIdx, parseResult);
 ...
 */
function runSimulation(startIdx, domElements, parseResult, simState, _generation) {
  if (!parseResult.toolpathSegments || parseResult.toolpathSegments.length === 0) return;
  var segments = parseResult.toolpathSegments;
  var idx = startIdx;
  function animate() {
    ...
    var speed = domElements.speedSlider ? Math.max(0.001, Number(domElements.speedSlider.value)) : 10;
    var segmentsThisFrame = speed > 1 ? Math.max(1, Math.round(speed)) : Math.max(1, Math.round(1 / speed));
...
var ViewerControls = /*#__PURE__*/_createClass(function ViewerControls(_viewer3d) {
...
/***/ "./src/client/js/visualizer/core.js":
/*!******************************************!*\
  !*** ./src/client/js/visualizer/core.js ***!
  \******************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   GcodeVisualizer: () => (/* binding */ GcodeVisualizer)
/* harmony export */ });
/* harmony import */ var _controls_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./controls.js */ "./src/client/js/visualizer/controls.js");
/* harmony import */ var _parser_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./parser.js */ "./src/client/js/visualizer/parser.js");
/* harmony import */ var _geometry_manager_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./geometry-manager.js */ "./src/client/js/visualizer/geometry-manager.js");
/* harmony import */ var _chunk_loader_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./chunk-loader.js */ "./src/client/js/visualizer/chunk-loader.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _slicedToArray(r, e) { return _arrayWithHoles(r) || _iterableToArrayLimit(r, e) || _unsupportedIterableToArray(r, e) || _nonIterableRest(); }
function _nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function _iterableToArrayLimit(r, l) { var t = null == r ? null : "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (null != t) { var e, n, i, u, a = [], f = !0, o = !1; try { if (i = (t = t.call(r)).next, 0 === l) { if (Object(t) !== t) return; f = !1; } else for (; !(f = (e = i.call(t)).done) && (a.push(e.value), a.length !== l); f = !0); } catch (r) { o = !0, n = r; } finally { try { if (!f && null != t["return"] && (u = t["return"](), Object(u) !== u)) return; } finally { if (o) throw n; } } return a; } }
 ...
 */
var GcodeVisualizer = /*#__PURE__*/function () {
  function GcodeVisualizer() {
    _classCallCheck(this, GcodeVisualizer);
    this.scene = null;
    this.camera = null;
    this.renderer = null;
    this.controls = null;
    ...
    this.geometryManager = new _geometry_manager_js__WEBPACK_IMPORTED_MODULE_2__.GeometryManager();
    this.chunkLoader = new _chunk_loader_js__WEBPACK_IMPORTED_MODULE_3__.ChunkLoader(this);
   ...
   */
  return _createClass(GcodeVisualizer, [{
    key: "setAdaptiveRendering",
    value: function setAdaptiveRendering(enabled) {
      var targetFPS = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 30;
    ...
    key: "shouldRenderFrame",
    value: function shouldRenderFrame(currentTime) {
      ...
      if (this.camera) {
        var cameraMovement = this.lastCameraPosition.distanceTo(this.camera.position);
    ...
    key: "initializeFrustumCulling",
    value: function initializeFrustumCulling() {
    ...
    key: "updatePerformanceMonitor",
    value: function updatePerformanceMonitor(currentTime, renderStartTime) {
      if (!this.performanceMonitor.enabled) return;
      this.performanceMonitor.frameCount++;
      var renderTime = currentTime - renderStartTime;

      // Add render time to history
      this.performanceMonitor.renderTimeHistory.push(renderTime);
      if (this.performanceMonitor.renderTimeHistory.length > this.performanceMonitor.maxHistoryLength) {
      ...
      if (this.performanceMonitor.lastTime === 0) {
        ...
        var timeDiff = currentTime - this.performanceMonitor.lastTime;
        if (timeDiff >= 1000) {
          this.performanceMonitor.averageFPS = Math.round(this.performanceMonitor.frameCount * 1000 / timeDiff);
      ...
      if (currentTime - this.performanceMonitor.lastLogTime > 1000 // Update every 1 second for live display
      ) {
        var avgRenderTime = this.performanceMonitor.renderTimeHistory.length > 0 ? this.performanceMonitor.renderTimeHistory.reduce(function (a, b) {
          return a + b;
        }, 0) / this.performanceMonitor.renderTimeHistory.length : 0;
    ...
    key: "isObjectVisible",
    value: function isObjectVisible(object) {
      var _this = this;
      ...
      if (object.geometry) {
        ...
        if (object.userData && object.userData.boundingBox) {
          var result = this.frustum.intersectsBox(object.userData.boundingBox);
          return result;
        }

        // Fallback to object's own bounding box if available
        if (object.geometry.boundingBox) {
          var _result = this.frustum.intersectsBox(object.geometry.boundingBox);
          return _result;
        }

        // Fallback to bounding sphere
        if (object.geometry.boundingSphere) {
          var _result2 = this.frustum.intersectsSphere(object.geometry.boundingSphere);
          return _result2;
        }
      }
      ...
      if (object.children && object.children.length > 0) {
        var _result3 = object.children.some(function (child) {
          return _this.isObjectVisible(child);
        });
        return _result3;
      }
    ...
    key: "applyFrustumCulling",
    value: function applyFrustumCulling() {
    ...
    value: function initialize(container) {
      ...
      this.perspectiveCamera = new THREE.PerspectiveCamera(60, container.offsetWidth / container.offsetHeight, 0.1, 50000);
      ...
      var frustumSize = 200; // Adjust this value based on your needs
      this.orthographicCamera = new THREE.OrthographicCamera(-frustumSize * aspect / 2, frustumSize * aspect / 2, frustumSize / 2, -frustumSize / 2, 0.1, 50000);
      ...
      this.renderer = new THREE.WebGLRenderer({
        ...
        precision: 'mediump',
        // Use medium precision for better performance
        stencil: false,
    ...
    value: function setupEventListeners() {
      ...
      var onResize = function onResize() {
        ...
        var w = _this2.container.offsetWidth;
        var h = _this2.container.offsetHeight;

        // Update camera aspect ratio for both cameras
        _this2.updateCameraAspect();
    ...
    key: "renderGcode",
    value: (function () {
      var _renderGcode = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(gcode) {
        var renderStartTime, result, renderEndTime, fileRenderTime, _t;
        return _regenerator().w(function (_context) {
          while (1) switch (_context.p = _context.n) {
            case 0:
              if (!(!gcode || !gcode.trim())) {
            ...
            case 1:
              ...
              _context.n = 3;
              return this.renderGcodeProgressive(gcode);
            case 3:
              result = _context.v;
              _context.n = 5;
              break;
            ...
            case 5:
              ...
              fileRenderTime = renderEndTime - renderStartTime; // Store file render time in global performance stats
              ...
              if (window.performanceData) {
                ...
                window.performanceData.lastFileRenderTime = new Date().toLocaleTimeString();
              }
      ...
      function renderGcode(_x) {
        return _renderGcode.apply(this, arguments);
      }
      return renderGcode;
    }()
    /**
    ...
    key: "renderGcodeImmediate",
    value: function renderGcodeImmediate(gcode) {
      var startTime = performance.now();

      // Parse G-code
      var parseStartTime = performance.now();
      this.parseResult = this.parseGcode(gcode);
      var parseTime = performance.now() - parseStartTime;
      ...
      window.performanceStats = {
        ...
        segmentCount: this.parseResult.toolpathSegments ? this.parseResult.toolpathSegments.length : 0
      };
    ...
    key: "renderGcodeProgressive",
    value: (function () {
      var _renderGcodeProgressive = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee2(gcode) {
        var _this3 = this;
        var result, _t2;
        return _regenerator().w(function (_context2) {
          while (1) switch (_context2.p = _context2.n) {
            case 0:
              ...
              this.loadingTimeout = setTimeout(function () {
                console.warn('Loading timeout reached - forcing hide of loading indicator');
                try {
              ...
              // Load G-code progressively
              _context2.n = 2;
              return this.chunkLoader.loadGcodeProgressive(gcode, {
                chunkSize: 1000,
                maxChunksPerFrame: 5,
                onProgress: function onProgress(progress) {
                  _this3.loadingProgress = progress.progress;
                  _this3.updateLoadingIndicator(progress);
                },
                onChunkProcessed: function onChunkProcessed(data) {
                  // Update visualization with partial data
                  _this3.updatePartialToolpath(data);
                }
              });
            case 2:
              result = _context2.v;
            ...
            case 4:
              ...
              this.parseResult = {
                ...
                segmentCounts: this.calculateSegmentCounts(result.segments || [], result.modes || []),
                lineMap: result.lineMap || [],
                // Use the correct line map from parser
              ...
              window.performanceStats = {
                parseTime: 0,
                // Progressive loading handles timing differently
                renderTime: 0,
            ...
            case 5:
              _context2.p = 5;
              _t2 = _context2.v;
              console.error('Progressive rendering failed:', _t2);
              // Clear the timeout
      ...
      function renderGcodeProgressive(_x2) {
        return _renderGcodeProgressive.apply(this, arguments);
      }
      return renderGcodeProgressive;
    }()
    /**
    ...
    key: "createToolpathLines",
    value: function createToolpathLines() {
      try {
        ...
        var g0Segments = [],
          ...
          g2Segments = [],
          g3Segments = [];
        var segments = this.parseResult.toolpathSegments;
        var modes = this.parseResult.toolpathModes;
        for (var i = 0; i < segments.length; i++) {
          var seg = segments[i];
          var mode = modes[i];
          if (mode === 'G0') g0Segments.push(seg);else if (mode === 'G1') g1Segments.push(seg);else if (mode === 'G2') g2Segments.push(seg);else if (mode === 'G3') g3Segments.push(seg);
        }
        ...
        this.createLineToolpathLines(g0Segments, g1Segments, g2Segments, g3Segments);
    ...
    key: "createInstancedToolpathLines",
    value: function createInstancedToolpathLines(g0Segments, g1Segments, g2Segments, g3Segments) {
      ...
      if (g0Segments.length > 0) {
        var g0Mesh = this.geometryManager.createInstancedLineSystem(g0Segments, 'G0');
        if (g0Mesh) {
      ...
      if (g1Segments.length > 0) {
        var g1Mesh = this.geometryManager.createInstancedLineSystem(g1Segments, 'G1');
        if (g1Mesh) {
      ...
      if (g2Segments.length > 0) {
        var g2Mesh = this.geometryManager.createInstancedLineSystem(g2Segments, 'G2');
        if (g2Mesh) {
      ...
      if (g3Segments.length > 0) {
        var g3Mesh = this.geometryManager.createInstancedLineSystem(g3Segments, 'G3');
        if (g3Mesh) {
    ...
    key: "setInstancedMeshBoundingBox",
    value: function setInstancedMeshBoundingBox(mesh, segments) {
      ...
      try {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            if (Array.isArray(segment) && segment.length >= 2) {
              ...
              var start = Array.isArray(segment[0]) ? new THREE.Vector3(segment[0][0], segment[0][1], segment[0][2]) : segment[0];
              var end = Array.isArray(segment[1]) ? new THREE.Vector3(segment[1][0], segment[1][1], segment[1][2]) : segment[1];
        ...
        if (points.length > 0) {
          ...
          var radius = 0.05; // Match the radius used in GeometryManager
    ...
    key: "createLineToolpathLines",
    value: function createLineToolpathLines(g0Segments, g1Segments, g2Segments, g3Segments) {
      // Convert segments to points format for LineSegments
      var g0Points = [];
      g0Segments.forEach(function (seg) {
        g0Points.push(seg[0], seg[1]);
      });
      var g1Points = [];
      g1Segments.forEach(function (seg) {
        g1Points.push(seg[0], seg[1]);
      });
      var g2Points = [];
      g2Segments.forEach(function (seg) {
        g2Points.push(seg[0], seg[1]);
      });
      var g3Points = [];
      g3Segments.forEach(function (seg) {
    ...
    key: "clearToolpathLines",
    value: function clearToolpathLines() {
      //console.log('Clearing toolpath lines');
      // Remove instanced meshes
      var _iterator2 = _createForOfIteratorHelper(this.instancedMeshes),
        _step2;
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      if (window.gcodeBatchedLines) {
        for (var mode in window.gcodeBatchedLines) {
          var line = window.gcodeBatchedLines[mode];
          if (line && line.parent) {
    ...
    value: function createLineObject(points, color, opacity, mode) {
      var _lineWidth = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
    ...
    key: "updatePartialToolpath",
    value: function updatePartialToolpath() {
    ...
    value: function showLoadingIndicator(show) {
      ...
      var loadingIndicator = document.getElementById('gcodeLoadingIndicator');
      if (show) {
        if (!loadingIndicator) {
    ...
    value: function updateLoadingIndicator(progress) {
      var loadingProgressBar = document.getElementById('loadingProgressBar');
      var loadingText = document.getElementById('loadingText');
      if (loadingProgressBar) {
        loadingProgressBar.style.width = "".concat(progress.progress, "%");
      }
      if (loadingText) {
        loadingText.textContent = "Processing: ".concat(Math.round(progress.progress), "% (").concat(progress.processedChunks, "/").concat(progress.totalChunks, " chunks)");
      }
    }
    ...
    key: "calculateSegmentCounts",
    value: function calculateSegmentCounts(segments, modes) {
      var counts = {
        G0: 0,
        G1: 0,
        G2: 0,
      ...
      var _iterator3 = _createForOfIteratorHelper(modes),
        _step3;
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
    ...
    key: "fitCameraToToolpath",
    value: function fitCameraToToolpath() {
      if (this.parseResult.toolpathPoints && this.parseResult.toolpathPoints.length > 0) {
        var box = new THREE.Box3().setFromPoints(this.parseResult.toolpathPoints);
        ...
        this.camera.position.set(center.x, center.y - cameraZ, center.z + cameraZ * 0.3);
    ...
    value: function switchCameraMode(mode) {
      ...
      var currentQuaternion = this.camera.quaternion.clone();
      var currentTarget = this.controls ? this.controls.target.clone() : null;
    ...
    value: function exportImage() {
      ...
      var imageData = this.renderer.domElement.toDataURL('image/png');

      // Create download link
      var link = document.createElement('a');
      link.href = imageData;
      link.download = "gcode_visualization_".concat(this.cameraMode, "_").concat(Date.now(), ".png");
      link.click();
    ...
    value: function updateCameraAspect() {
      ...
      if (this.camera.isPerspectiveCamera) {
        ...
        var frustumSize = 200; // Match the value used in initialization
    ...
    key: "setupSimulationControls",
    value: function setupSimulationControls() {
      var domElements = {
      ...
      if (domElements.playBtn && domElements.progressBar) {
        (0,_controls_js__WEBPACK_IMPORTED_MODULE_0__.setupSimulationControls)(domElements, this.parseResult);
      }
    }
    ...
    value: function startAnimationLoop() {
      ...
      var _animate = function animate() {
        ...
        if (_this4.controls) _this4.controls.update();
        _this4.render();
      };

      // Store render function globally for simulation
      window.gcodeRender = function () {
    ...
    value: function render() {
      try {
        if (this.renderer && this.scene && this.camera) {
          ...
          var shouldRender = this.shouldRenderFrame(now);
          if (!shouldRender) {
    ...
    key: "parseGcode",
    value: function parseGcode(gcode) {
      // Use the existing parser from the legacy code
      return (0,_parser_js__WEBPACK_IMPORTED_MODULE_1__.parseGcodeOptimized)(gcode);
    }
    ...
    key: "toggleOptimization",
    value: function toggleOptimization(optimization, enabled) {
      switch (optimization) {
        ...
        case 'frustumCulling':
          ...
          break;
        default:
          console.warn("Unknown optimization: ".concat(optimization));
      }
    }
    ...
    value: function getPerformanceStats() {
      return {
        averageFPS: this.performanceMonitor.averageFPS,
        averageRenderTime: this.performanceMonitor.renderTimeHistory.length > 0 ? (this.performanceMonitor.renderTimeHistory.reduce(function (a, b) {
          return a + b;
        }, 0) / this.performanceMonitor.renderTimeHistory.length).toFixed(2) + 'ms' : 'N/A',
        culledObjects: this.culledObjects.size,
        totalObjects: this.scene ? this.scene.children.length : 0,
        adaptiveRendering: this.adaptiveRendering,
        optimizations: {
...
window.optimizationControls = {
  toggleAdaptiveRendering: function toggleAdaptiveRendering(enabled) {
    if (window.gcodeVisualizer) {
      window.gcodeVisualizer.toggleOptimization('adaptiveRendering', enabled);
    }
  },
  toggleFrustumCulling: function toggleFrustumCulling(enabled) {
    if (window.gcodeVisualizer) {
      window.gcodeVisualizer.toggleOptimization('frustumCulling', enabled);
    }
  },
  getPerformanceStats: function getPerformanceStats() {
    if (window.gcodeVisualizer) {
  ...
  enableAllOptimizations: function enableAllOptimizations() {
    if (window.gcodeVisualizer) {
  ...
  disableAllOptimizations: function disableAllOptimizations() {
    if (window.gcodeVisualizer) {
...
/***/ "./src/client/js/visualizer/geometry-manager.js":
/*!******************************************************!*\
  !*** ./src/client/js/visualizer/geometry-manager.js ***!
  \******************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   GeometryManager: () => (/* binding */ GeometryManager)
/* harmony export */ });
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _createForOfIteratorHelper(r, e) { var t = "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (!t) { if (Array.isArray(r) || (t = _unsupportedIterableToArray(r)) || e && r && "number" == typeof r.length) { t && (r = t); var _n = 0, F = function F() {}; return { s: F, n: function n() { return _n >= r.length ? { done: !0 } : { done: !1, value: r[_n++] }; }, e: function e(r) { throw r; }, f: F }; } throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); } var o, a = !0, u = !1; return { s: function s() { t = t.call(r); }, n: function n() { var r = t.next(); return a = r.done, r; }, e: function e(r) { u = !0, o = r; }, f: function f() { try { a || null == t["return"] || t["return"](); } finally { if (u) throw o; } } }; }
function _unsupportedIterableToArray(r, a) { if (r) { if ("string" == typeof r) return _arrayLikeToArray(r, a); var t = {}.toString.call(r).slice(8, -1); return "Object" === t && r.constructor && (t = r.constructor.name), "Map" === t || "Set" === t ? Array.from(r) : "Arguments" === t || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(t) ? _arrayLikeToArray(r, a) : void 0; } }
 ...
 */
var GeometryManager = /*#__PURE__*/function () {
   ...
   */
  return _createClass(GeometryManager, [{
    key: "createInstancedLineSystem",
    value: function createInstancedLineSystem(segments, mode) {
      try {
        var lineCount = segments.length;
        if (lineCount === 0) return null;
        console.log("Creating instanced line system for ".concat(lineCount, " ").concat(mode, " segments"));

        // Create base geometry (cylinder for lines)
        var radius = 0.05; // Reduced for better visibility
        var geometry = this.getOrCreateBaseGeometry('cylinder', radius);
        ...
        console.log("Created material for ".concat(mode, " with color ").concat(this.getModeColor(mode).toString(16), " and opacity ").concat(this.getModeOpacity(mode)));

        // Create instanced mesh
        var instancedMesh = new THREE.InstancedMesh(geometry, material, lineCount);
        console.log("Created instanced mesh with capacity for ".concat(lineCount, " instances"));

        // Calculate matrices for each segment
        for (var i = 0; i < segments.length; i++) {
          var segment = segments[i];
          var matrix = this.calculateCylinderMatrix(segment[0], segment[1]);
        ...
        console.log("Instanced line system created successfully for ".concat(mode), {
        ...
        console.error("Error creating instanced line system for ".concat(mode, ":"), error);
        return null;
      }
    }
    ...
    key: "getOrCreateBaseGeometry",
    value: function getOrCreateBaseGeometry(type) {
      var radius = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0.05;
      var key = "".concat(type, "-").concat(radius);
      ...
      switch (type) {
        case 'cylinder':
          ...
          console.log("Created cylinder geometry with radius ".concat(radius, ", height 1"));
          break;
        default:
    ...
    key: "calculateCylinderMatrix",
    value: function calculateCylinderMatrix(start, end) {
      // Convert array points to Vector3 if needed
      var startPoint = Array.isArray(start) ? new THREE.Vector3(start[0], start[1], start[2]) : start;
      var endPoint = Array.isArray(end) ? new THREE.Vector3(end[0], end[1], end[2]) : end;
      ...
      if (length === 0) {
        ...
        console.log("Zero-length segment at ".concat(startPoint.x, ", ").concat(startPoint.y, ", ").concat(startPoint.z));
        return matrix;
      }
      ...
      var up = new THREE.Vector3(0, 1, 0); // Default cylinder orientation
      quaternion.setFromUnitVectors(up, direction.clone().normalize());

      // Scale to correct length (cylinder is 1 unit tall, so we scale Y to length)
      var scale = new THREE.Vector3(1, length, 1);

      // Compose all transformations into one matrix
      matrix.compose(midpoint, quaternion, scale);
      console.log("Cylinder: length=".concat(length, ", position=(").concat(midpoint.x, ", ").concat(midpoint.y, ", ").concat(midpoint.z, ")"));
      return matrix;
    }
    ...
    key: "getModeColor",
    value: function getModeColor(mode) {
      var colors = {
        G0: 0xff8e37,
        // Orange (rapid)
        G1: 0x00ff99,
    ...
    key: "getModeOpacity",
    value: function getModeOpacity(mode) {
      var opacities = {
        G0: 0.6,
        // Rapid moves are less prominent
        G1: 0.9,
        G2: 0.9,
        G3: 0.9
      };
    ...
    key: "updateInstancedLineSystem",
    value: function updateInstancedLineSystem(segments, mode) {
      var instancedMesh = this.instancedMeshes.get(mode);
      ...
      for (var _i = segments.length; _i < instancedMesh.count; _i++) {
        var _matrix = new THREE.Matrix4();
        _matrix.makeScale(0, 0, 0); // Scale to zero to hide
        instancedMesh.setMatrixAt(_i, _matrix);
      }
      instancedMesh.instanceMatrix.needsUpdate = true;
      return true;
    }
    ...
    value: function dispose() {
      ...
      var _iterator = _createForOfIteratorHelper(this.baseGeometries.values()),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
...
/***/ "./src/client/js/visualizer/index.js":
/*!*******************************************!*\
  !*** ./src/client/js/visualizer/index.js ***!
  \*******************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   GcodeVisualizer: () => (/* reexport safe */ _core_js__WEBPACK_IMPORTED_MODULE_0__.GcodeVisualizer),
/* harmony export */   ViewerControls: () => (/* reexport safe */ _controls_js__WEBPACK_IMPORTED_MODULE_3__.ViewerControls),
/* harmony export */   closeGcodeViewer: () => (/* reexport safe */ _modal_js__WEBPACK_IMPORTED_MODULE_2__.closeGcodeViewer),
/* harmony export */   parseGcodeOptimized: () => (/* reexport safe */ _parser_js__WEBPACK_IMPORTED_MODULE_1__.parseGcodeOptimized),
/* harmony export */   setupSimulationControls: () => (/* reexport safe */ _controls_js__WEBPACK_IMPORTED_MODULE_3__.setupSimulationControls),
/* harmony export */   showGcodeViewer: () => (/* reexport safe */ _modal_js__WEBPACK_IMPORTED_MODULE_2__.showGcodeViewer)
/* harmony export */ });
/* harmony import */ var _core_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./core.js */ "./src/client/js/visualizer/core.js");
/* harmony import */ var _parser_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./parser.js */ "./src/client/js/visualizer/parser.js");
/* harmony import */ var _modal_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./modal.js */ "./src/client/js/visualizer/modal.js");
/* harmony import */ var _controls_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./controls.js */ "./src/client/js/visualizer/controls.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function ownKeys(e, r) { var t = Object.keys(e); if (Object.getOwnPropertySymbols) { var o = Object.getOwnPropertySymbols(e); r && (o = o.filter(function (r) { return Object.getOwnPropertyDescriptor(e, r).enumerable; })), t.push.apply(t, o); } return t; }
function _objectSpread(e) { for (var r = 1; r < arguments.length; r++) { var t = null != arguments[r] ? arguments[r] : {}; r % 2 ? ownKeys(Object(t), !0).forEach(function (r) { _defineProperty(e, r, t[r]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : ownKeys(Object(t)).forEach(function (r) { Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r)); }); } return e; }
function _defineProperty(e, r, t) { return (r = _toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
...
function updateGcodeStats() {
  ...
  var statsContainer = document.getElementById('gcodeViewerStats');
  if (!statsContainer) {
    return;
  }
  ...
  // Get existing stats
  var perf = window.performanceStats || {};
  var segCounts = window.gcodeSegmentCounts || {};
  ...
  if (window.gcodeVisualizer && window.gcodeVisualizer.getPerformanceStats) {
    try {
      var vizStats = window.gcodeVisualizer.getPerformanceStats();
      performanceData = _objectSpread(_objectSpread({}, performanceData), vizStats);
    } catch (error) {
  ...
  var statsHtml = "\n    <div class=\"stats-title\">\n      <div class=\"metric-line\">Lines: ".concat(perf.lineCount || 0, "</div>\n      <div class=\"metric-line\">Segments: ").concat(perf.segmentCount || 0, "</div>\n      <div class=\"metric-line metric-good\">G0: ").concat(segCounts.G0 || 0, "</div>\n      <div class=\"metric-line metric-good\">G1: ").concat(segCounts.G1 || 0, "</div>\n      <div class=\"metric-line metric-info\">G2: ").concat(segCounts.G2 || 0, "</div>\n      <div class=\"metric-line metric-info\">G3: ").concat(segCounts.G3 || 0, "</div>");

  // Add FPS information
  if (performanceData.averageFPS !== undefined && performanceData.averageFPS > 0) {
    var fpsClass = performanceData.averageFPS >= 30 ? 'metric-good' : performanceData.averageFPS >= 15 ? 'metric-warning' : 'metric-error';
    statsHtml += "<div class=\"metric-line ".concat(fpsClass, "\">FPS: ").concat(performanceData.averageFPS, "</div>");
  } else {
  ...
  if (performanceData.fileRenderTime !== undefined && performanceData.fileRenderTime > 0) {
    var fileRenderTime = typeof performanceData.fileRenderTime === 'number' ? performanceData.fileRenderTime : 0;
    var renderClass = fileRenderTime <= 1000 ? 'metric-good' : fileRenderTime <= 3000 ? 'metric-warning' : 'metric-error';
    statsHtml += "<div class=\"metric-line ".concat(renderClass, "\">File Render: ").concat(fileRenderTime.toFixed(1), "ms</div>");
  } else if (perf.fileRenderTime !== undefined && perf.fileRenderTime > 0) {
    var _fileRenderTime = typeof perf.fileRenderTime === 'number' ? perf.fileRenderTime : 0;
    var _renderClass = _fileRenderTime <= 1000 ? 'metric-good' : _fileRenderTime <= 3000 ? 'metric-warning' : 'metric-error';
    statsHtml += "<div class=\"metric-line ".concat(_renderClass, "\">File Render: ").concat(_fileRenderTime.toFixed(1), "ms</div>");
  } else {
  ...
  if (performanceData.totalObjects !== undefined) {
    statsHtml += "<div class=\"metric-line metric-info\">Objects: ".concat(performanceData.totalObjects, "</div>");
  }
  if (performanceData.culledObjects !== undefined && performanceData.culledObjects > 0) {
    statsHtml += "<div class=\"metric-line metric-info\">Culled: ".concat(performanceData.culledObjects, "</div>");
  }

  // Add canvas size
  if (performanceData.canvasSize) {
    statsHtml += "<div class=\"metric-line metric-info\">Canvas: ".concat(performanceData.canvasSize.width, "\xD7").concat(performanceData.canvasSize.height, "</div>");
  }

  // Add memory information
  if (performanceData.memoryInfo) {
    var memUsage = performanceData.memoryInfo.used / performanceData.memoryInfo.total * 100;
    var memClass = memUsage < 70 ? 'metric-good' : memUsage < 85 ? 'metric-warning' : 'metric-error';
    statsHtml += "<div class=\"metric-line ".concat(memClass, "\">Memory: ").concat(performanceData.memoryInfo.used, "/").concat(performanceData.memoryInfo.total, "MB</div>");
  }

  // Add adaptive rendering status
  if (performanceData.adaptiveRendering !== undefined) {
    var adaptClass = performanceData.adaptiveRendering ? 'metric-good' : 'metric-info';
    statsHtml += "<div class=\"metric-line ".concat(adaptClass, "\">Adaptive: ").concat(performanceData.adaptiveRendering ? 'ON' : 'OFF', "</div>");
  }

  // Add last update time
  if (performanceData.lastUpdate) {
    statsHtml += "<div class=\"timestamp\">Updated: ".concat(performanceData.lastUpdate, "</div>");
  }
  statsHtml += "</div>";
...
window.gcodeRender = function (orig) {
  ...
  var statsUpdateInterval = 1000; // Update stats only once per second during animation

  return function () {
    if (typeof orig === 'function') orig();

    // Only update stats if canvas is present and enough time has passed
    var canvas = document.querySelector('#gcodeViewerContainer canvas');
    if (canvas) {
      var now = performance.now();
...
window.GcodeVisualizer = _core_js__WEBPACK_IMPORTED_MODULE_0__.GcodeVisualizer;
window.ViewerControls = _controls_js__WEBPACK_IMPORTED_MODULE_3__.ViewerControls;
...
window.testLineMappingFix = function () {
  ...
  var arcSegments = [];
  for (var i = 0; i < window.gcodeLineMap.length; i++) {
    var lineNum = window.gcodeLineMap[i];
    if (lineNum >= 0 && lineNum < window.gcodeLines.length) {
      var gcodeLine = window.gcodeLines[lineNum];
  ...
  if (arcSegments.length > 0) {
    ...
    arcSegments.forEach(function (arc) {
      console.log("  Segment ".concat(arc.segment, " -> Line ").concat(arc.line + 1, ": ").concat(arc.gcode));
    });
  } else {
...
window.switchCameraMode = function () {
  if (window.gcodeVisualizer && typeof window.gcodeVisualizer.switchCameraMode === 'function') {
    var newMode = window.gcodeVisualizer.cameraMode === 'perspective' ? 'orthographic' : 'perspective';
...
window.debugCurrentLine = function () {
  var currentSeg = window.gcodeCurrentLineIdx || 0;
  var lineNum = window.gcodeLineMap ? window.gcodeLineMap[currentSeg] : -1;
  var gcodeLine = lineNum >= 0 && window.gcodeLines ? window.gcodeLines[lineNum] : 'N/A';
  console.log("Current: Segment ".concat(currentSeg, " -> Line ").concat(lineNum + 1, ": ").concat(gcodeLine));
};

/***/ }),

/***/ "./src/client/js/visualizer/modal.js":
/*!*******************************************!*\
  !*** ./src/client/js/visualizer/modal.js ***!
  \*******************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   GcodeViewerModal: () => (/* binding */ GcodeViewerModal),
/* harmony export */   closeGcodeViewer: () => (/* binding */ closeGcodeViewer),
/* harmony export */   showGcodeViewer: () => (/* binding */ showGcodeViewer)
/* harmony export */ });
/* harmony import */ var _core_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./core.js */ "./src/client/js/visualizer/core.js");
function _typeof(o) { "@babel/helpers - typeof"; return _typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, _typeof(o); }
function _regenerator() { /*! regenerator-runtime -- Copyright (c) 2014-present, Facebook, Inc. -- license (MIT): https://github.com/babel/babel/blob/main/packages/babel-helpers/LICENSE */ var e, t, r = "function" == typeof Symbol ? Symbol : {}, n = r.iterator || "@@iterator", o = r.toStringTag || "@@toStringTag"; function i(r, n, o, i) { var c = n && n.prototype instanceof Generator ? n : Generator, u = Object.create(c.prototype); return _regeneratorDefine2(u, "_invoke", function (r, n, o) { var i, c, u, f = 0, p = o || [], y = !1, G = { p: 0, n: 0, v: e, a: d, f: d.bind(e, 4), d: function d(t, r) { return i = t, c = 0, u = e, G.n = r, a; } }; function d(r, n) { for (c = r, u = n, t = 0; !y && f && !o && t < p.length; t++) { var o, i = p[t], d = G.p, l = i[2]; r > 3 ? (o = l === n) && (u = i[(c = i[4]) ? 5 : (c = 3, 3)], i[4] = i[5] = e) : i[0] <= d && ((o = r < 2 && d < i[1]) ? (c = 0, G.v = n, G.n = i[1]) : d < l && (o = r < 3 || i[0] > n || n > l) && (i[4] = r, i[5] = n, G.n = l, c = 0)); } if (o || r > 1) return a; throw y = !0, n; } return function (o, p, l) { if (f > 1) throw TypeError("Generator is already running"); for (y && 1 === p && d(p, l), c = p, u = l; (t = c < 2 ? e : u) || !y;) { i || (c ? c < 3 ? (c > 1 && (G.n = -1), d(c, u)) : G.n = u : G.v = u); try { if (f = 2, i) { if (c || (o = "next"), t = i[o]) { if (!(t = t.call(i, u))) throw TypeError("iterator result is not an object"); if (!t.done) return t; u = t.value, c < 2 && (c = 0); } else 1 === c && (t = i["return"]) && t.call(i), c < 2 && (u = TypeError("The iterator does not provide a '" + o + "' method"), c = 1); i = e; } else if ((t = (y = G.n < 0) ? u : r.call(n, G)) !== a) break; } catch (t) { i = e, c = 1, u = t; } finally { f = 1; } } return { value: t, done: y }; }; }(r, o, i), !0), u; } var a = {}; function Generator() {} function GeneratorFunction() {} function GeneratorFunctionPrototype() {} t = Object.getPrototypeOf; var c = [][n] ? t(t([][n]())) : (_regeneratorDefine2(t = {}, n, function () { return this; }), t), u = GeneratorFunctionPrototype.prototype = Generator.prototype = Object.create(c); function f(e) { return Object.setPrototypeOf ? Object.setPrototypeOf(e, GeneratorFunctionPrototype) : (e.__proto__ = GeneratorFunctionPrototype, _regeneratorDefine2(e, o, "GeneratorFunction")), e.prototype = Object.create(u), e; } return GeneratorFunction.prototype = GeneratorFunctionPrototype, _regeneratorDefine2(u, "constructor", GeneratorFunctionPrototype), _regeneratorDefine2(GeneratorFunctionPrototype, "constructor", GeneratorFunction), GeneratorFunction.displayName = "GeneratorFunction", _regeneratorDefine2(GeneratorFunctionPrototype, o, "GeneratorFunction"), _regeneratorDefine2(u), _regeneratorDefine2(u, o, "Generator"), _regeneratorDefine2(u, n, function () { return this; }), _regeneratorDefine2(u, "toString", function () { return "[object Generator]"; }), (_regenerator = function _regenerator() { return { w: i, m: f }; })(); }
 ...
 */
var GcodeViewerModal = /*#__PURE__*/function () {
  function GcodeViewerModal() {
    _classCallCheck(this, GcodeViewerModal);
    this.modal = null;
    this.container = null;
   ...
   */
  return _createClass(GcodeViewerModal, [{
    key: "initialize",
    value: function initialize() {
      ...
      this.visualizer = new _core_js__WEBPACK_IMPORTED_MODULE_0__.GcodeVisualizer();
    ...
    value: function show(gcode) {
      ...
      var gcodeContent = gcode;
      if (!gcodeContent && window.outputEditor && typeof window.outputEditor.getValue === 'function') {
      ...
      setTimeout(function () {
        _this.renderGcode(gcodeContent);
      }, 100);
    }
      ...
      var _renderGcode = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(gcode) {
        ...
        return _regenerator().w(function (_context) {
          while (1) switch (_context.p = _context.n) {
            ...
            case 3:
              _context.n = 4;
              return this.visualizer.renderGcode(gcode);
            case 4:
              result = _context.v;
              if (result.success) {
                _context.n = 5;
            ...
            case 6:
              _context.p = 6;
              _t = _context.v;
              console.error('Error rendering G-code:', _t);
              this.showError("Failed to render G-code: ".concat(_t.message));
            case 7:
              return _context.a(2);
          }
        }, _callee, this, [[2, 6]]);
 ...
 */
function showGcodeViewer(gcode) {
  if (!modalInstance) {
...
/***/ "./src/client/js/visualizer/parser.js":
/*!********************************************!*\
  !*** ./src/client/js/visualizer/parser.js ***!
  \********************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   parseGcodeOptimized: () => (/* binding */ parseGcodeOptimized)
/* harmony export */ });
function _toConsumableArray(r) { return _arrayWithoutHoles(r) || _iterableToArray(r) || _unsupportedIterableToArray(r) || _nonIterableSpread(); }
function _nonIterableSpread() { throw new TypeError("Invalid attempt to spread non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function _unsupportedIterableToArray(r, a) { if (r) { if ("string" == typeof r) return _arrayLikeToArray(r, a); var t = {}.toString.call(r).slice(8, -1); return "Object" === t && r.constructor && (t = r.constructor.name), "Map" === t || "Set" === t ? Array.from(r) : "Arguments" === t || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(t) ? _arrayLikeToArray(r, a) : void 0; } }
 ...
 */
function interpolateArc(start, end, center, isClockwise) {
  var segments = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 32;
  var points = [];
  ...
  for (var i = 0; i <= segments; i++) {
    ...
    var zVal = start.z === end.z ? start.z : start.z + (end.z - start.z) * i / segments;
    points.push(new THREE.Vector3(center.x + radius * Math.cos(angle), center.y + radius * Math.sin(angle), zVal));
  }
  return points;
}
...
function parseGcodeOptimized(gcode) {
  ...
  try {
    var lines = gcode.split(/\r?\n/).map(function (line) {
      return line.trim().replace(GCODE_REGEX.N_LINE, '');
    }).filter(function (line) {
      return line.length > 0;
    });

    // Use initial state or defaults - G-code maintains state between lines
    var x = initialState.x !== undefined ? initialState.x : 0;
    var y = initialState.y !== undefined ? initialState.y : 0;
    var z = initialState.z !== undefined ? initialState.z : 0;
    var currentMotionMode = initialState.currentMotionMode || null;
    var anyDrawn = initialState.anyDrawn || false;
    var toolpathPoints = [];
    var toolpathSegments = [];
    var toolpathModes = [];
    var lineMap = [];
    var segmentCounts = {
      G0: 0,
    ...
    lines.forEach(function (line, idx) {
      ...
      var lineWithoutComments = line.replace(GCODE_REGEX.COMMENT, '').trim();
      ...
      var allModes = _toConsumableArray(lineWithoutComments.matchAll(GCODE_REGEX.G_MODE));
      if (allModes.length > 0) {
        currentMotionMode = allModes[allModes.length - 1][0];
      }
      var hasCoords = GCODE_REGEX.HAS_COORDS.test(lineWithoutComments);
      ...
      var matchX = lineWithoutComments.match(GCODE_REGEX.X_COORD);
      var matchY = lineWithoutComments.match(GCODE_REGEX.Y_COORD);
      var matchZ = lineWithoutComments.match(GCODE_REGEX.Z_COORD);
      var matchI = lineWithoutComments.match(GCODE_REGEX.I_OFFSET);
      var matchJ = lineWithoutComments.match(GCODE_REGEX.J_OFFSET);
      var matchR = lineWithoutComments.match(GCODE_REGEX.R_RADIUS);

      // G-code behavior: if coordinate not specified, use previous value
      var targetX = matchX ? parseFloat(matchX[1]) : x;
      var targetY = matchY ? parseFloat(matchY[1]) : y;
      var targetZ = matchZ ? parseFloat(matchZ[1]) : z;
      if (currentMotionMode === 'G2' || currentMotionMode === 'G3') {
        // Arc processing
        var arcStart = new THREE.Vector3(x, y, z);
        var arcEnd = new THREE.Vector3(targetX, targetY, targetZ);
        var isClockwise = currentMotionMode === 'G2';
        var arcPoints = [];
        if (matchI && matchJ) {
          var center = new THREE.Vector3(x + parseFloat(matchI[1]), y + parseFloat(matchJ[1]), z);
          ...
          var r = Math.abs(parseFloat(matchR[1]));
          if (d > 0 && !isNaN(r) && r > 0) {
            var h = Math.sqrt(Math.max(0, r * r - d / 2 * (d / 2)));
            var mx = (x + targetX) / 2;
            var my = (y + targetY) / 2;
            var sign = (isClockwise ? -1 : 1) * (parseFloat(matchR[1]) >= 0 ? 1 : -1);
            var cx = mx + sign * h * (dy / d);
            var cy = my - sign * h * (dx / d);
            if (!isNaN(cx) && !isNaN(cy)) {
              var _center = new THREE.Vector3(cx, cy, z);
              arcPoints = interpolateArc(arcStart, arcEnd, _center, isClockwise);
            }
          }
        }
        if (arcPoints.length > 1) {
          ...
          var lastPoint = arcPoints[arcPoints.length - 1].clone();
          x = lastPoint.x;
          y = lastPoint.y;
          z = lastPoint.z;
        ...
        if (toolpathPoints.length >= 2) {
          var newSegment = [startPoint.clone(), endPoint.clone()];
    ...
    var finalState = {
      ...
      y: y,
      z: z,
      currentMotionMode: currentMotionMode,
      anyDrawn: anyDrawn
    };
